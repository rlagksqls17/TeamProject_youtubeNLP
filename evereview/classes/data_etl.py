...
class review_etl:
    def extract(self, video_id=None, channel_id=None, day_start=None, day_end=None):
        ...
        if video_id:
            ...
            result_df['publishedAt'] = result_df['publishedAt'].apply(lambda x: int(x[0:4] + x[5:7] + x[8:10]))
        ...
        # 댓글 기간별 수집
            ...
            # 쿼리문
            result_df = result_df[(result_df['publishedAt'] >= day_start) & (result_df['publishedAt'] <= day_end)].reset_index().iloc[:, 1:]
            comments = result_df.values.tolist()
